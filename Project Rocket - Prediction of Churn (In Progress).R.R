...
preprocesseddata3 <- subset(log_data, select = c(AfterHours,MonthlyIncome,TenureCompany,JobDedication,Department,FacilitiesSatisfaction,LastPromotion,NumCompaniesWorked,New_Churn)) #top 8, cutoff at 10%
...
test <- glm(New_Churn ~ AfterHours + JobDedication + TenureCompany + MonthlyIncome + FacilitiesSatisfaction + LastPromotion + NumCompaniesWorked + Department, family = "binomial", data=trainDatatest)
