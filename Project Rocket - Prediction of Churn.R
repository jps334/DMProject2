...
setwd('C:/Riot Games/Data Mining 2')
getwd()
library('readxl')
...
summary(rawdata$NumCompaniesWorked) #
...
count(rawdata$NumberProjectsLastYear) 
...
#was used for testing with dummy variables
library(dummies)
rawdatadum <- rawdata
rawdatadum1 <- dummy.data.frame(rawdatadum, 'JobRole')

rawdatadum2 <- dummy.data.frame(rawdatadum1, 'Education')

rawdatadum3 <- dummy.data.frame(rawdatadum2, 'EducationArea')

rawdatadum4 <- dummy.data.frame(rawdatadum3, 'Department')

rawdatadum5 <- dummy.data.frame(rawdatadum4, 'JobRole')

rawdatadum6 <- dummy.data.frame(rawdatadum5, 'Generation')
rawdatadum <- rawdatadum6

rm(rawdatadum1,rawdatadum2,rawdatadum3,rawdatadum4,rawdatadum5,rawdatadum5,rawdatadum6)
...
hist(originaldata$`SalaryRise(%)`, main = "Percentage of Salary Rise",
     col = "lightblue2",
     xlab = "Salary Rise",
     ylab= "Frequency",
     breaks= 10:25,
...
barplot(table(originaldata$Dependents), main= "Number of dependents",
        col= "lightblue2",
...
barplot(table(originaldata$JobLevel), main= "Job level",
        col= "peachpuff",
        xlab = "Job Level",
        ylab= "Frequency",
        ylim=c(0,600)) 
...
barplot(table(originaldata$NumCompaniesWorked), main= "Number of Companies Worked",
        col= "honeydew2",
...
boxplot(originaldata$MonthlyIncome,
        horizontal=TRUE, cex.axis=.9,
        xlab= "Monthly Income", col="lightgrey")


barplot(table(originaldata$TenureWorking), main= "Tenure Working",
        col= "peachpuff",
        xlab = "Tenure Working",
        ylab= "Frequency",
        ylim=c(0,250)) 

barplot(table(originaldata$TenureCompany), main= "Tenure Company",
        col= "lightblue2",
        xlab = "Tenure Company",
        ylab= "Frequency",
        ylim=c(0,250)) 

barplot(table(originaldata$TenureRole), main= "Tenure Role",
        col= "honeydew2",
        xlab = "Tenure Role",
        ylab= "Frequency",
        ylim=c(0,400)) 
...
barplot(table(originaldata$TenureManager), main= "Tenure Manager",
        col= "peachpuff",
        xlab = "Tenure Manager",
        ylab= "Frequency",
        ylim=c(0,400)) 
...
slices<- c(sum(originaldata$JobDedication==1),sum(originaldata$JobDedication==2),
           sum(originaldata$JobDedication==3),sum(originaldata$JobDedication==4))
lbls<- c("Low", "Medium", "High", "Very High")
pie(slices, labels = lbls, main = "Job Dedication")



slices<- c(sum(originaldata$JobPerformance==1),sum(originaldata$JobPerformance==2),
           sum(originaldata$JobPerformance==3),sum(originaldata$JobPerformance==4))
lbls<- c("Very Low", "Low", "Medium", "High")
pie(slices, labels = lbls, main = "Job Performance")



slices<- c(sum(originaldata$FacilitiesSatisfaction==1),sum(originaldata$FacilitiesSatisfaction==2),
           sum(originaldata$FacilitiesSatisfaction==3),sum(originaldata$FacilitiesSatisfaction==4))
lbls<- c("Bad", "Satisfactory", "Good", "Very good")
pie(slices, labels = lbls, main = "Facilities Satisfaction")


slices<- c(sum(originaldata$RoleSatisfaction==1),sum(originaldata$RoleSatisfaction==2),
           sum(originaldata$RoleSatisfaction==3),sum(originaldata$RoleSatisfaction==4))
lbls<- c("Low", "Medium", "High", "Very High")
pie(slices, labels = lbls, main = "Role Satisfaction")


slices<- c(sum(originaldata$HierarchySatisfaction==1),sum(originaldata$HierarchySatisfaction==2),
           sum(originaldata$HierarchySatisfaction==3),sum(originaldata$HierarchySatisfaction==4))
lbls<- c("Low", "Medium", "High", "Very High")
pie(slices, labels = lbls, main = "Hierarchy Satisfaction")


barplot(table(originaldata$NumberProjectsLastYear), main= "Number of Projects Last Year",
        col= "honeydew2",
        xlab = "Number of Projects",
        ylab= "Frequency",
        ylim=c(0,600)) 

barplot(table(originaldata$LastPromotion), main= "Years since Last Promotion",
        col= "lightblue2",
        xlab = "Years",
        ylab= "Frequency",
        ylim=c(0,600)) 
...
hist(originaldata$DistanceHomeOffice, main = "Distance Home to Office", col = "peachpuff",
     xlab = "Kms")


table(originaldata$Gender)

slices<- c(sum(originaldata$Gender=="Female"),sum(originaldata$Gender=="Male"))
lbls<- c("Female", "Male")
...
library(ggplot2)
ggplot(originaldata, aes(x=Gender)) + geom_bar()


#proporcao de homens e mulheres por departamento
ggplot(rawdata, aes(x = Department, fill = Gender)) + geom_bar(position = "fill")


ggplot(rawdata, aes(x=Department)) + geom_bar()

ggplot(rawdata, aes(x=JobType)) + geom_bar()


table(originaldata$JobRole)


table(originaldata$`Marital Status`)

ggplot(rawdata, aes(x = AfterHours, fill = Churn)) + geom_bar(position = "dodge")

slices<- c(sum(originaldata$Churn=="Yes"),sum(originaldata$Churn=="No"))
lbls<- c("Yes", "No")
pie(slices, labels = lbls, main = "Churn")


slices<- c(sum(originaldata$BalanceWork.Life=="Bad"),sum(originaldata$BalanceWork.Life=="Medium"),
           sum(originaldata$BalanceWork.Life=="Good"),sum(originaldata$BalanceWork.Life=="Great"))
lbls<- c("Bad", "Medium", "Good", "Great")
pie(slices, labels = lbls, main = "Balance Work & Life")

table(originaldata$TypeContract)

ggplot(rawdata, aes(x=Education)) + geom_bar()

ggplot(rawdata, aes(x=EducationArea)) + geom_bar()



#Graficos comparacao com Churn

#Dodge

ggplot(rawdata, aes(x = JobRole, fill = Churn)) + geom_bar(position = "dodge") 

ggplot(rawdata, aes(x = JobDedication, fill = Churn)) + geom_bar(position = "dodge")  # print

ggplot(rawdata, aes(x = FacilitiesSatisfaction, fill = Churn)) + geom_bar(position = "dodge") #print

ggplot(rawdata, aes(x = New_MaritalStatus, fill = Churn)) + geom_bar(position = "dodge") #print

ggplot(rawdata, aes(x = New_JobType, fill = Churn)) + geom_bar(position = "dodge") #print

ggplot(rawdata, aes(x = TenureCompany, fill = Churn)) + geom_bar(position = "dodge") #print

ggplot(rawdata, aes(x = MonthlyIncome, fill = Churn)) + geom_bar(position = "dodge") 

ggplot(rawdata, aes(x = Department, fill = Churn)) + geom_bar(position = "dodge") #print

ggplot(rawdata, aes(x = LastPromotion, fill = Churn)) + geom_bar(position = "dodge") #print

ggplot(rawdata, aes(x = NumCompaniesWorked, fill = Churn)) + geom_bar(position = "dodge") #print


#Fill


ggplot(rawdata, aes(x = JobRole, fill = Churn)) + geom_bar(position = "fill") #print

ggplot(rawdata, aes(x = JobDedication, fill = Churn)) + geom_bar(position = "fill")

ggplot(rawdata, aes(x = FacilitiesSatisfaction, fill = Churn)) + geom_bar(position = "fill")

ggplot(rawdata, aes(x = New_MaritalStatus, fill = Churn)) + geom_bar(position = "fill")

ggplot(rawdata, aes(x = New_JobType, fill = Churn)) + geom_bar(position = "fill")

ggplot(rawdata, aes(x = TenureCompany, fill = Churn)) + geom_bar(position = "fill")

ggplot(rawdata, aes(x = MonthlyIncome, fill = Churn)) + geom_bar(position = "fill")

ggplot(rawdata, aes(x = Department, fill = Churn)) + geom_bar(position = "fill")

ggplot(rawdata, aes(x = LastPromotion, fill = Churn)) + geom_bar(position = "fill")

ggplot(rawdata, aes(x = NumCompaniesWorked, fill = Churn)) + geom_bar(position = "fill")
...
rawdatadum10 <- subset(rawdatadum, rawdatadum$TenureCompany<=30)
rawdatadum9 <- subset(rawdatadum10, rawdatadum10$Age<=65)
rawdatadum9$Dependents <- ifelse(rawdatadum9$Dependents>3, 1, rawdatadum9$Dependents)
rawdatadum9$LastPromotion <- ifelse(rawdatadum9$LastPromotion>14, 1, rawdatadum9$LastPromotion)
rawdatadum <- rawdatadum9
rm(rawdatadum9,rawdatadum10)
...
log_data <- log_data[order(log_data$EmployeeID),]


log_datadum <- rawdatadum
log_datadum$MonthlyIncome <-log(rawdatadum$MonthlyIncome)
log_datadum$originalsalary <-log(rawdatadum$originalsalary)
log_datadum$TenureCompany <-  (rawdatadum$TenureCompany-min(rawdatadum$TenureCompany))/(max(rawdatadum$TenureCompany)-min(rawdatadum$TenureCompany))
log_datadum$LastPromotion <-  (rawdatadum$LastPromotion-min(rawdatadum$LastPromotion))/(max(rawdatadum$LastPromotion)-min(rawdatadum$LastPromotion))
log_datadum <- log_datadum[order(log_datadum$EmployeeID),]


#the dum uses the dummy variables created previously for a test
preprocesseddatadum<- subset(log_datadum, select = -c(Age,Dependents,agetest,year_birth, AvgSatisfaction, originalsalary, AfterHours, EmployeeID, BirthDate, Gender, `Marital Status`, JobType, Churn, New_MaritalStatus, New_JobType, New_Dependents, BalanceWork.Life))
...
preprocesseddata3 <- subset(log_data, select = c(AfterHours,MonthlyIncome,JobDedication,TenureCompany,New_MaritalStatus,NumCompaniesworked,FacilitiesSatisfaction,LastPromotion,New_Churn)) 
preprocesseddata4 <- subset(log_data, select = c(AfterHours,JobRole,New_MaritalStatus,JobDedication,FacilitiesSatisfaction,New_JobType,New_Churn)) 
preprocesseddatadum <- subset(preprocesseddatadum, select = -c(JobLevel, TenureWorking, TenureRole, `SalaryRise(%)`, TenureManager))
...
pca_datadum <- prcomp(subset(preprocesseddatadum, select = -c(New_Churn)), center = TRUE, scale. = TRUE)
...
pca4dum <- pca_datadum$x[,1:5]
pca4dum[,5] <- preprocesseddatadum$New_Churn
colnames(pca4dum)[5] <- 'Churn'
pca4dum <- as.data.frame(pca4dum)
...
trainingRowIndexdum <- sample(1:nrow(preprocesseddatadum), 0.7*nrow(preprocesseddatadum))
trainDatadum <- preprocesseddatadum[trainingRowIndexdum, ]
testDatadum <- preprocesseddatadum[-trainingRowIndexdum, ]
...
trainData2 <- preprocesseddata2[trainingRowIndex2, ]
testData2 <- preprocesseddata2[-trainingRowIndex2, ]

trainingRowIndex4 <- sample(1:nrow(pca4), 0.7*nrow(pca4))
trainData4 <-pca4[trainingRowIndex4, ]
testData4 <- pca4[-trainingRowIndex4, ]

trainingRowIndex4dum <- sample(1:nrow(pca4dum), 0.7*nrow(pca4dum))
trainData4dum <-pca4dum[trainingRowIndex4dum, ]
testData4dum <- pca4dum[-trainingRowIndex4dum, ]
...
logitmod4dum <- glm(Churn ~ PC1 + PC2 + PC3 + PC4, family = "binomial", data=trainData4dum)
...
test <- glm(New_Churn ~ ., family = "binomial", data=trainDatatest)
anova <- glm(New_Churn ~ ., family = "binomial", data=trainDataanova)
...
pred4dum <- predict(logitmod4dum, testData4dum, type = "response")
...
y_pred4dum_num <- ifelse(pred4dum > 0.5, 1, 0)
y_pred4dum <- factor(y_pred4dum_num, levels=c(0, 1))
y_act4dum <- testData4dum$Churn
...
caret::confusionMatrix(y_predtest, as.factor(y_acttest), positive = '1')
caret::confusionMatrix(y_predanova, as.factor(y_actanova), positive = '1')
caret::confusionMatrix(y_pred, as.factor(y_act), positive = '1')
caret::confusionMatrix(y_pred4, as.factor(y_act4), positive = '1')
caret::confusionMatrix(y_pred4dum, as.factor(y_act4dum), positive = '1')
caret::confusionMatrix(y_pred10, as.factor(y_act10), positive = '1')
caret::confusionMatrix(y_pred20, as.factor(y_act20), positive = '1')
...
model4dum <- NaiveBayes(factor(Churn)~. , data = trainData4dum, fL = 1)
...
x_test4dum <- testData4dum[,1:4]
y_test4dum <- testData4dum[,5]
predictions4dum <- predict(model4dum, x_test4dum)
...
y_testtest <- testDatatest[,9]
predictionstest <- predict(modeltest, x_testtest)

#summarize results
caret::confusionMatrix(predictions4$class, as.factor(y_test4), positive = '1')
caret::confusionMatrix(predictions4dum$class, as.factor(y_test4dum), positive = '1')
caret::confusionMatrix(predictions10$class, as.factor(y_test10), positive = '1')
caret::confusionMatrix(predictions20$class, as.factor(y_test20), positive = '1')
caret::confusionMatrix(predictions$class, as.factor(y_test), positive = '1')
caret::confusionMatrix(predictionstest$class, as.factor(y_testtest), positive = '1')
caret::confusionMatrix(predictionsanova$class, as.factor(y_testanova), positive = '1')
...
library(gmodels)
gmodels::CrossTable(tree_pred, testData$New_Churn,
                    prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE,
                    dnn = c('actual churn', 'predicted churn'))
caret::confusionMatrix(tree_pred, as.factor(testData$New_Churn), positive = '1')
...
caret::confusionMatrix(forest_pred, as.factor(testData2$New_Churn), positive = '1')
...
caret::confusionMatrix(knn_test_pred4, as.factor(testData4$Churn), positive = '1')

knn_test_pred4dum <- knn(train = trainData4dum, test = testData4dum, cl = trainData4$Churn, k = 16)
caret::confusionMatrix(knn_test_pred4dum, as.factor(testData4dum$Churn), positive = '1')


knn_test_pred10 <- knn(train = trainData10, test = testData10, cl = trainData10$Churn, k = 16)
caret::confusionMatrix(knn_test_pred10, as.factor(testData10$Churn), positive = '1')

knn_test_pred20 <- knn(train = trainData20, test = testData20, cl = trainData20$Churn, k = 16)
caret::confusionMatrix(knn_test_pred20, as.factor(testData20$Churn), positive = '1')
...
caret::confusionMatrix(knn_test_pred2, as.factor(testData2$New_Churn), positive = '1')

knn_test_preddum <- knn(train = trainDatadum, test = testDatadum, cl = trainDatadum$New_Churn, k = 16)
caret::confusionMatrix(knn_test_preddum, as.factor(testDatadum$New_Churn), positive = '1')
...
nn4dum <- neuralnet(Churn ~ PC1 + PC2 + PC3 + PC4, data=trainData4dum, linear.output = F, hidden = 3)
...
#checked cutoff before
validation.prediction=compute(nn, testData2[,-c(13)])
y_prednn_num <- ifelse(validation.prediction$net.result > 0.5, 1, 0)
y_prednn <- factor(y_prednn_num, levels=c(0, 1))
confusionMatrix(y_prednn, as.factor(testData2$New_Churn))
...
y_pred4nn <- factor(y_pred4nn_num, levels=c(0, 1))
confusionMatrix(y_pred4nn, as.factor(y_act4))

hist(validation4dum.prediction$net.result)
validation4dum.prediction=compute(nn4dum, testData4dum[,-c(5)])
y_pred4dumnn_num <- ifelse(validation4dum.prediction$net.result > 0.5, 1, 0)
y_pred4dumnn <- factor(y_pred4dumnn_num, levels=c(0, 1))
confusionMatrix(y_pred4dumnn, as.factor(y_act4dum))


validation10.prediction=compute(nn10, testData10[,-c(11)])
y_pred10nn_num <- ifelse(validation10.prediction$net.result > 0.5, 1, 0)
y_pred10nn <- factor(y_pred10nn_num, levels=c(0, 1))
confusionMatrix(y_pred10nn, as.factor(y_act10))

validation20.prediction=compute(nn20, testData20[,-c(21)])
y_pred20nn_num <- ifelse(validation20.prediction$net.result > 0.5, 1, 0)
y_pred20nn <- factor(y_pred20nn_num, levels=c(0, 1))
confusionMatrix(y_pred20nn, as.factor(y_act20))


#SVN
library("e1071")

svm_model1 <- svm(as.factor(New_Churn)~.,data = trainData)
pred <- predict(svm_model1, testData)
confusionMatrix(pred, as.factor(y_act))



svm_modeltest <- svm(as.factor(New_Churn)~.,data = trainDatatest)
predtest <- predict(svm_modeltest, testDatatest)
confusionMatrix(predtest, as.factor(y_acttest))

svm_modelanova <- svm(as.factor(New_Churn)~.,data = trainDataanova)
predanova <- predict(svm_modelanova, testDataanova)
confusionMatrix(predanova, as.factor(y_actanova))

svm_model10 <- svm(as.factor(Churn)~.,data = trainData10)
pred10 <- predict(svm_model10, testData10)
confusionMatrix(pred10, as.factor(y_act10))

svm_model4 <- svm(as.factor(Churn)~.,data = trainData4)
pred4 <- predict(svm_model4, testData4)
confusionMatrix(pred4, as.factor(y_act4))


svm_model4dum <- svm(as.factor(Churn)~.,data = trainData4dum)
pred4dum <- predict(svm_model4dum, testData4dum)
confusionMatrix(pred4dum, as.factor(y_act4dum))

svm_model20 <- svm(as.factor(Churn)~.,data = trainData20)
pred20 <- predict(svm_model20, testData20)
confusionMatrix(pred20, as.factor(y_act20))
...
#Neural Network
#NOPCA Only numeric
r <- roc(testData2$New_Churn, as.numeric(validation.prediction$net.result))
...
#PCA4
r4 <- roc(y_act4, validation4.prediction$net.result)
...
#PCA10
r10 <- roc(y_act10, validation10.prediction$net.result)
...
#PCA20
r20 <- roc(y_act20, validation20.prediction$net.result)
